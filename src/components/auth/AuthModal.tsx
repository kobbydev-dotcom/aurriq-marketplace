...
export function AuthModal({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  return (
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        <Dialog.Overlay className="fixed inset-0 z-[100] bg-black/80 transition-opacity" />
        <Dialog.Content className="fixed top-1/2 left-1/2 z-[110] w-full max-w-md -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-2xl border border-[#c9930a]/25 bg-[#0c0904] p-6 text-[#f0eae0] shadow-[0_24px_90px_rgba(0,0,0,0.7)] outline-none transition-all max-h-[90vh]">
