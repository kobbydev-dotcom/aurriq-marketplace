...
export default function InventoryTab() {
  const rawInventory = useQuery(api.inventory.getInventoryReport, {});
  const stats = useQuery(api.products.listAll, {}) as any;
  const [restockTarget, setRestockTarget] = useState<InventoryItem | null>(null);

  if (rawInventory === undefined || stats === undefined) {
    return (
  ...
  // Safe data-transform adaptation step to bridge mismatch
  const inventory: InventoryItem[] = (rawInventory || []).map((p: any) => ({
    ...p,
    brand: p.brand || "Aurriq",
    stockStatus: p.stockQuantity === 0 ? "out" : (p.isLowStock || p.stockQuantity <= p.lowStockThreshold) ? "low" : "ok"
  }));
