...
export const sendReceiptEmail = internalAction({
  ...
  handler: async (_ctx, args) => {
    ...
    const html = `<div style="font-family:Georgia,serif;max-width:520px;margin:0 auto;border:1px solid #eee;border-radius:12px;overflow:hidden;"><div style="background:#0c0904;padding:20px 24px;"><span style="color:#c9930a;font-size:20px;letter-spacing:4px;">AURRIQ</span></div><div style="padding:24px;"><p style="font-size:16px;color:#1a1a1a;">Hi ${args.name},</p><p style="font-size:14px;color:#555;">Thank you for your order. Here is your receipt.</p><table style="width:100%;border-collapse:collapse;margin-top:12px;">${row("Reference", args.reference)}${row("Item", `${args.productName} × ${args.quantity}`)}${row("Seller", args.sellerName)}${row("Payment method", args.paymentMethod)}<tr><td colspan="2" style="border-top:1px solid #eee;padding-top:8px;"></td></tr>${row("Total", args.total)}${row("Amount paid", args.amountPaid)}${row("Balance on delivery", args.balance)}</table><p style="font-size:12px;color:#999;margin-top:20px;">Keep this receipt for your records and use only the seller payment details shown during checkout.</p></div></div>`;
