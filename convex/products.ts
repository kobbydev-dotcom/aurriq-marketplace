...
export const createProduct = mutation({
  args: {
    ...
    // UPGRADE: Accept the variant tracking array from the creation form
    variants: v.optional(
      v.array(
        v.object({
          color: v.string(),
  ...
  handler: async (ctx, args) => {
    ...
    // UPGRADE: Automatically calculate total stock quantity by adding up all variants
    const totalCalculatedStock = args.variants
      ? args.variants.reduce((sum, variant) => sum + variant.stock, 0)
      : 0;
    ...
    return await ctx.db.insert("products", {
      ...
      stockQuantity: totalCalculatedStock, // Calculated automatically
      variants: args.variants,             // Saved to database
...
export const updateProduct = mutation({
  args: {
    ...
    // UPGRADE: Allow updating variant lists
    variants: v.optional(
      v.array(
        v.object({
          color: v.string(),
  ...
  handler: async (ctx, args) => {
    ...
    // UPGRADE: Recalculate global stock if variants are altered during an edit
    let updatedStockQuantity = product.stockQuantity;
    if (updates.variants) {
      updatedStockQuantity = updates.variants.reduce((sum, variant) => sum + variant.stock, 0);
    }

    const newThreshold = updates.lowStockThreshold ?? product.lowStockThreshold;
    const resetLowStockAlert = updatedStockQuantity > newThreshold ? { lowStockAlertSent: false } : {};
    const resetOutOfStockAlert = updatedStockQuantity > 0 ? { outOfStockAlertSent: false } : {};

    await ctx.db.patch(productId, { 
      ...updates, 
      stockQuantity: updatedStockQuantity,
      ...resetLowStockAlert, 
      ...resetOutOfStockAlert 
    });
  },
});
...
export const listProducts = query({
  args: {
    ...
    // UPGRADE: Allow filtering by a specific booking app owner
    sellerId: v.optional(v.id("users")), 
  },
  handler: async (ctx, args) => {
    let productsQuery;

    // If a sellerId is provided (came from the Booking App link), grab only their inventory
    if (args.sellerId) {
