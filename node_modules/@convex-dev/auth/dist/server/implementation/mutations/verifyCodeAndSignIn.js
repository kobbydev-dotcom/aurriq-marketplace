...
export async function verifyCodeAndSignInImpl(ctx, args, getProviderOrThrow, config) {
    ...
    if (identifier !== undefined) {
        if (await isSignInRateLimited(ctx, identifier, config)) {
            ...
            logWithLevel(LOG_LEVELS.WARN, "Too many failed attempts to verify code for this email");
            return null;
        }
    }
...
methodProviderId, getProviderOrThrow, allowExtraProviders, config, sessionId) {
    ...
    if (account === null) {
        ...
        logWithLevel(LOG_LEVELS.WARN, "Account associated with this email has been deleted");
        return null;
    }
    if (methodProviderId !== null &&
        ...
        logWithLevel(LOG_LEVELS.WARN, `Invalid provider "${methodProviderId}" for given \`code\`, ` +
