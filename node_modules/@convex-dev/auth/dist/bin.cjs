...
function isUnicodeSupported() {
  if (!import_node_process3.default.platform.startsWith("win")) {
    return import_node_process3.default.env["TERM"] !== "linux";
  }
  return Boolean(import_node_process3.default.env["CI"]) || // CI environments generally support unicode
  Boolean(import_node_process3.default.env["WT_SESSION"]) || // Windows Terminal
  Boolean(import_node_process3.default.env["TERMINUS_SUBLIME"]) || // Terminus (<0.2.27)
  import_node_process3.default.env["ConEmuTask"] === "{cmd::Cmder}" || // ConEmu and cmder
...
var defaultTheme = {
  prefix: {
    idle: (0, import_node_util3.styleText)("blue", "?"),
    done: (0, import_node_util3.styleText)("green", dist_default.tick)
  },
  spinner: {
    interval: 80,
    frames: ["\u280B", "\u2819", "\u2839", "\u2838", "\u283C", "\u2834", "\u2826", "\u2827", "\u2807", "\u280F"].map((frame) => (0, import_node_util3.styleText)("yellow", frame))
  },
  keybindings: [],
  style: {
    answer: (text) => (0, import_node_util3.styleText)("cyan", text),
    message: (text) => (0, import_node_util3.styleText)("bold", text),
    error: (text) => (0, import_node_util3.styleText)("red", `> ${text}`),
    defaultAnswer: (text) => (0, import_node_util3.styleText)("dim", `(${text})`),
    help: (text) => (0, import_node_util3.styleText)("dim", text),
    highlight: (text) => (0, import_node_util3.styleText)("cyan", text),
    key: (text) => (0, import_node_util3.styleText)("cyan", (0, import_node_util3.styleText)("bold", `<${text}>`))
  }
};
...
function wrapAnsi(string2, columns, options) {
  return String(string2).normalize().split(CRLF_OR_LF).map((line) => exec(line, columns, options)).join("\n");
}
...
var ScreenManager = class {
  ...
  render(content, bottomContent = "") {
    ...
    const rawPromptLine = (0, import_node_util4.stripVTControlCharacters)(promptLine);
...
var Separator = class {
  separator = (0, import_node_util5.styleText)("dim", Array.from({ length: 15 }).join(dist_default.line));
  type = "separator";
...
var checkboxTheme = {
  icon: {
    checked: (0, import_node_util6.styleText)("green", dist_default.circleFilled),
    unchecked: dist_default.circle,
    cursor: dist_default.pointer,
    disabledChecked: (0, import_node_util6.styleText)("green", dist_default.circleDouble),
    disabledUnchecked: "-"
  },
  style: {
...
var dist_default4 = createPrompt((config, done) => {
  ...
  if (status === "done") {
    ...
    return [prefix, message2, answer].filter(Boolean).join(" ");
  }
  const keys = [
    ["\u2191\u2193", "navigate"],
    ["space", "select"]
...
var ExternalEditor = class {
  ...
  run() {
    ...
    try {
      try {
        const editorProcess = (0, import_node_child_process2.spawnSync)(this.editor.bin, this.editorArgs(), {
          shell: false,
          stdio: "inherit"
        });
  ...
  runAsync(callback) {
    ...
    const promise = new Promise((resolve, reject) => {
      try {
        const editorProcess = (0, import_node_child_process2.spawn)(this.editor.bin, this.editorArgs(), {
          shell: false,
          stdio: "inherit"
        });
  ...
  cleanup() {
    if (!this.tempDir)
      return;
    try {
      (0, import_node_fs2.rmSync)(this.tempDir, { force: true, recursive: true });
      this.tempFile = "";
      this.tempDir = "";
  ...
  createTempFile() {
    try {
      const baseDir = import_node_path3.default.resolve(this.fileOptions.dir ?? import_node_os2.default.tmpdir());
      this.tempDir = (0, import_node_fs2.mkdtempSync)(import_node_path3.default.join(baseDir, "inquirer-editor-"));
      const id = (0, import_node_crypto.randomUUID)();
      const prefix = sanitizeAffix(this.fileOptions.prefix);
      const postfix = sanitizeAffix(this.fileOptions.postfix);
      const filename = `${prefix}${id}${postfix}`;
      this.tempFile = import_node_path3.default.join(this.tempDir, filename);
      const opt = { encoding: "utf8", flag: "wx" };
      if (Object.prototype.hasOwnProperty.call(this.fileOptions, "mode")) {
        opt.mode = this.fileOptions.mode;
      }
      (0, import_node_fs2.writeFileSync)(this.tempFile, this.text, opt);
  ...
  readTemporaryFile() {
    try {
      const tempFileBuffer = (0, import_node_fs2.readFileSync)(this.tempFile);
...
var dist_default5 = createPrompt((config, done) => {
  ...
  return [[prefix, message2, helpTip].filter(Boolean).join(" "), error];
});
...
var expand = createPrompt((config, done) => {
    ...
    if (isEnterKey(event)) {
      ...
      if (answer === "h" && !expanded) {
        ...
        if (selectedChoice) {
          ...
          setError(`"${(0, import_node_util7.styleText)("red", value)}" isn't an available option`);
        }
      }
    } else {
      setValue(rl.line);
  ...
  if (status === "done") {
    ...
    return `${prefix} ${message2} ${theme.style.answer(selectedChoice.name)}`;
  }
  ...
  if (currentOption) {
    helpTip = `${(0, import_node_util7.styleText)("cyan", ">>")} ${currentOption.name}`;
  }
  let error = "";
  if (errorMsg) {
    error = theme.style.error(errorMsg);
  ...
  useKeypress((key, rl) => {
    if (isEnterKey(key)) {
      ...
      if (isSelectableChoice(selectedChoice2)) {
        ...
        setError(`"${(0, import_node_util8.styleText)("red", value)}" isn't an available option`);
      }
    } else if (isUpKey(key, keybindings2) || isDownKey(key, keybindings2)) {
      rl.clearLine(0);
...
var dist_default11 = createPrompt((config, done) => {
  ...
  return [[prefix, message2, config.mask ? formattedValue : helpTip].join(" "), error];
});
...
var searchTheme = {
  ...
  style: {
    disabled: (text) => (0, import_node_util9.styleText)("dim", `- ${text}`),
    searchTerm: (text) => (0, import_node_util9.styleText)("cyan", text),
    description: (text) => (0, import_node_util9.styleText)("cyan", text),
    keysHelpTip: (keys) => keys.map(([key, action]) => `${(0, import_node_util9.styleText)("bold", key)} ${(0, import_node_util9.styleText)("dim", action)}`).join((0, import_node_util9.styleText)("dim", " \u2022 "))
  }
};
...
var dist_default12 = createPrompt((config, done) => {
  ...
  if (status === "done" && selectedChoice) {
    return [prefix, message2, theme.style.answer(selectedChoice.short)].filter(Boolean).join(" ").trimEnd();
  } else {
    searchStr = theme.style.searchTerm(searchTerm);
  }
  const description = selectedChoice?.description;
  const header = [prefix, message2, searchStr].filter(Boolean).join(" ").trimEnd();
  const body = [
    error ?? page,
    " ",
    description ? theme.style.description(description) : "",
...
var selectTheme = {
  icon: { cursor: dist_default.pointer },
  style: {
    disabled: (text) => (0, import_node_util10.styleText)("dim", text),
    description: (text) => (0, import_node_util10.styleText)("cyan", text),
    keysHelpTip: (keys) => keys.map(([key, action]) => `${(0, import_node_util10.styleText)("bold", key)} ${(0, import_node_util10.styleText)("dim", action)}`).join((0, import_node_util10.styleText)("dim", " \u2022 "))
  },
...
// node_modules/inquirer/dist/utils/observable.js
...
var PromptsRunner = class {
  ...
  prepareQuestion = async (question) => {
    ...
    const [message2, defaultValue, resolvedChoices] = await Promise.all([
  ...
  fetchAnswer = async (rawQuestion) => {
    ...
    const promptFn = isPromptConstructor(prompt2) ? (q, opt) => new Promise((resolve, reject) => {
      ...
      const onClose = () => {
        ...
        import_node_readline.default.cursorTo(rl.output, 0);
        rl.output.end();
        rl.close();
      };
...
var genericExport = (keyType, keyFormat, key) => {
  ...
  if (isCryptoKey(key)) {
    ...
    keyObject = import_node_crypto2.KeyObject.from(key);
...
var keyToJWK = (key) => {
  ...
  if (isCryptoKey(key)) {
    ...
    keyObject = import_node_crypto3.KeyObject.from(key);
  } else if (is_key_object_default(key)) {
    keyObject = key;
...
var generate = (0, import_node_util11.promisify)(import_node_crypto4.generateKeyPair);
...
async function generateKeys() {
  try {
    const keys = await generateKeyPair2("RS256");
...
// src/cli/index.ts
new Command().name("@convex-dev/auth").description(
...
).option("--skip-git-check", "Don't warn when running outside a Git checkout.").option(
  "--web-server-url <url>",
  ...
  const usesTypeScript = !!(packageJson.dependencies?.typescript || packageJson.devDependencies?.typescript);
...
async function configureSiteUrl(config, forcedValue) {
  ...
  const value = config.deployment.type === "dev" || config.deployment.type === null ? config.isVite ? "http://localhost:5173" : "http://localhost:3000" : void 0;
  const description = config.deployment.type === "dev" ? "the URL of your local web server (e.g. http://localhost:1234)" : "the URL where your site is hosted (e.g. https://example.com)";
...
async function configureEnvVar(config, variable) {
  ...
  if (existing !== "") {
    if (!await promptForConfirmation(
      `The ${printDeployment(config)} already has ${variable.name} configured to ${source_default.bold(existing)}. Do you want to change it?`,
      { default: false }
    )) {
      return;
    }
...
async function backendEnvVar(config, name) {
  return (0, import_child_process.execSync)(`npx convex env get ${deploymentOptions(config)} ${name}`, {
    stdio: "pipe"
  }).toString().slice(0, -1);
}
async function setEnvVar(config, name, value, options) {
  ...
  if (options?.hideValue) {
    logSuccess(
      `Successfully set ${source_default.bold(name)} (on ${printDeployment(config)})`
    );
  }
}
function deploymentOptions(config) {
  const {
    deployment: {
      options: { adminKey }
    }
  } = config;
  const adminKeyOption = adminKey !== void 0 ? `--admin-key '${adminKey}' ` : "";
  return adminKeyOption + deploymentNameOptions(config);
}
function deploymentNameOptions(config) {
  const {
    deployment: {
...
function printDeployment(config) {
  const { name, type } = config.deployment;
  return (type !== null ? `${source_default.bold(type)} ` : "") + "deployment" + (name !== null ? ` ${source_default.bold(name)}` : "");
}
var compilerOptionsPattern = /("compilerOptions"\s*:\s*\{(?:\s*(?:\/\*(?:[^*]|\*(?!\/))*\*\/))*(\s*))(?=")/;
var validTsConfig = `{
...
async function modifyTsConfig(config) {
  ...
  const tsConfigPath = import_path.default.join(config.convexFolderPath, "tsconfig.json");
  if (!(0, import_fs.existsSync)(tsConfigPath)) {
    if ((0, import_fs.existsSync)(projectLevelTsConfigPath)) {
      if (config.isExpo) {
        (0, import_fs.writeFileSync)(tsConfigPath, validTsConfig);
  ...
  const existingTsConfig = (0, import_fs.readFileSync)(tsConfigPath, "utf8");
  ...
  if (/Bundler/i.test(existingModuleResolution) && existingSkipLibCheck === "true") {
    logSuccess(`The ${source_default.bold(tsConfigPath)} is already set up.`);
    return;
  }
  if (!compilerOptionsPattern.test(existingTsConfig)) {
    logInfo(
      `Modify your ${source_default.bold(tsConfigPath)} to include the following:`
    );
    const source = `  {
    "compilerOptions": {
      "moduleResolution": "Bundler",
...
`));
  ...
  (0, import_fs.writeFileSync)(tsConfigPath, changedTsConfig);
...
`;
  ...
  if (existingConfigPath !== null) {
    const existingConfig = (0, import_fs.readFileSync)(existingConfigPath, "utf8");
...
`));
    ...
    (0, import_fs.writeFileSync)(newConfigPath, source);
...
async function initializeAuth(config) {
  ...
  const sourceTemplate = `import { convexAuth } from "@convex-dev/auth/server";
...
`;
  const source = templateToSource(sourceTemplate);
  const authPath = import_path.default.join(config.convexFolderPath, "auth");
  const existingAuthPath = await existingNonEmptySourcePath(authPath);
  if (existingAuthPath !== null) {
    const existingAuth = (0, import_fs.readFileSync)(existingAuthPath, "utf8");
    if (doesAlreadyMatchTemplate(existingAuth, sourceTemplate)) {
      ...
      logInfo(
        `You already have a ${source_default.bold(existingAuthPath)}, make sure it initializes \`convexAuth\` like this:`
      );
      print(indent(`
${source}
`));
...
`;
  const source = templateToSource(sourceTemplate);
  const httpPath = import_path.default.join(config.convexFolderPath, "http");
  const existingHttpPath = await existingNonEmptySourcePath(httpPath);
  if (existingHttpPath !== null) {
    const existingHttp = (0, import_fs.readFileSync)(existingHttpPath, "utf8");
    if (doesAlreadyMatchTemplate(existingHttp, sourceTemplate)) {
      ...
      logInfo(
        `You already have a ${source_default.bold(existingHttpPath)}, make sure it includes the call to \`auth.addHttpRoutes\`:`
      );
      print(indent(`
${source}
`));
    ...
    (0, import_fs.writeFileSync)(newHttpPath, source);
    logSuccess(`Created ${source_default.bold(newHttpPath)}`);
  }
}
var VariablesSchema = object({
...
async function configureOtherVariables(config, json) {
  const variables = parse(VariablesSchema, JSON.parse(json));
  logStep(config, "Configure extra environment variables");
  if (variables.help !== void 0) {
...
async function existingNonEmptySourcePath(path5) {
  return await existsAndNotEmpty(`${path5}.ts`) ? `${path5}.ts` : await existsAndNotEmpty(`${path5}.js`) ? `${path5}.js` : null;
}
async function existsAndNotEmpty(path5) {
  return (0, import_fs.existsSync)(path5) && (0, import_fs.readFileSync)(path5, "utf8").trim() !== "";
}
function logStep(config, message2) {
  if (config.step > 1) {
    print();
  }
  logInfo(source_default.bold(`Step ${config.step++}: ${message2}`));
}
async function checkSourceControl(options) {
  if (options.skipGitCheck) {
    return;
  }
  const isGit = (0, import_fs.existsSync)(".git");
  if (!isGit) {
    logWarning(
...
function readPackageJson() {
  try {
    const data = (0, import_fs.readFileSync)("package.json", "utf8");
    return JSON.parse(data);
  } catch (error) {
    logErrorAndExit(
      '`@convex-dev/auth` must be run from a project directory which includes a valid "package.json" file. You can create one by running `npm init`.',
      error.message
    );
  }
}
function readConvexJson() {
  if (!(0, import_fs.existsSync)("convex.json")) {
    return {};
  }
  try {
    const data = (0, import_fs.readFileSync)("convex.json", "utf8");
    return JSON.parse(data);
  } catch (error) {
    logErrorAndExit(
...
async function promptForConfirmationOrExit(message2, options = {}) {
  if (!await promptForConfirmation(message2, options)) {
    process.exit(1);
  }
}
async function promptForConfirmation(message2, options = {}) {
...
async function promptForInput(message2, options) {
  if (process.stdout.isTTY) {
    const { input } = await dist_default14.prompt([
      {
        type: "input",
        name: "input",
        message: message2,
...
function logErrorAndExit(message2, error) {
  logError(message2, error);
  process.exit(1);
}
function logError(message2, error) {
  print(
    `${source_default.red(`\u2716`)} ${indent(message2)}${error !== void 0 ? `
  ${source_default.grey(`Error: ${indent(error)}`)}` : ""}`
  );
}
function logWarning(message2) {
  print(`${source_default.yellow.bold(`!`)} ${indent(message2)}`);
}
function logInfo(message2) {
  print(`${source_default.blue.bold(`i`)} ${indent(message2)}`);
}
function logSuccess(message2) {
  print(`${source_default.green(`\u2714`)} ${indent(message2)}`);
}
function print(message2) {
  process.stderr.write((message2 ?? "") + "\n");
}
function indent(string2) {
  return string2.replace(/^/gm, "  ").slice(2);
}
